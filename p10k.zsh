# Generated by Powerlevel10k configuration wizard on 2024-03-27 at 17:46 KST.
...
#    # example               # example user-defined segment (see prompt_example function below)
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local    default='%6F'  # cyan foreground
    else
    ...
    if [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]]; then
      ...
      if [[ -n $namerev ]]; then
        ...
        res+="${meta}@${default}${VCS_STATUS_COMMIT[1,8]}"
      fi
    fi

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${meta}:${default}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
